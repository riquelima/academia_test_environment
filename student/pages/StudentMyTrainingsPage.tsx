  ...
  return (
    <Modal isOpen={!!training} onClose={onClose} title={`Detalhes: ${training.name}`}>
      <div className="space-y-3 sm:space-y-4 text-sm">
        <div className="relative h-32 sm:h-40 w-full rounded-lg overflow-hidden mb-2 sm:mb-3">
            <img 
                src={training.imageUrl || 'https://source.unsplash.com/500x300/?fitness,gym'} 
                alt={training.name} 
                className="absolute inset-0 w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent p-2 sm:p-3 flex flex-col justify-end">
                <h3 className="text-base sm:text-md font-semibold text-white">{training.name}</h3>
            </div>
        </div>

        <div className="mb-1">
            <strong className="text-xs text-medium-text-light dark:text-medium-text">Objetivos:</strong>
         ...
         <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-xs text-medium-text-light dark:text-medium-text">
        ...
        <div className="space-y-2 sm:space-y-3 pt-2 sm:pt-3 border-t border-light-border dark:border-dark-border">
          <h4 className="text-sm sm:text-base font-semibold text-brand-orange">Divisões e Exercícios:</h4>
          {training.splits.map(split => (
            <div key={split.id} className="p-2 sm:p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-light-border dark:border-dark-border">
              <h5 className="text-xs sm:text-sm font-medium text-dark-text dark:text-light-text mb-1 sm:mb-2">{split.customName}</h5>
              {split.exercises.length > 0 ? (
                <ul className="space-y-1 sm:space-y-1.5 text-xs">
                  {split.exercises.map(ex => (
                    <li key={ex.id} className="p-1 sm:p-1.5 bg-gray-100 dark:bg-gray-700 rounded-md border border-gray-200 dark:border-gray-600">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-semibold text-dark-text dark:text-light-text flex items-center">
          <IconWeightLiftingUp className="w-7 h-7 sm:w-8 sm:h-8 mr-2 sm:mr-3 text-brand-orange" />
          Meus Treinos
        </h2>
      </div>
      ...
      {assignedTrainings.length === 0 ? (
        <div className="bg-light-bg-card dark:bg-dark-card p-6 sm:p-8 rounded-xl shadow-lg text-center">
          <IconWeightLiftingUp className="w-12 h-12 sm:w-16 sm:h-16 mx-auto text-gray-400 dark:text-gray-500 mb-4" />
          <p className="text-lg sm:text-xl font-semibold text-dark-text dark:text-light-text mb-2">Nenhum treino atribuído.</p>
          <p className="text-sm text-medium-text-light dark:text-medium-text">Entre em contato com seu instrutor para receber um plano de treino.</p>
        </div>
      ) : (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {training.objective.slice(0, 2).map(obj => (
                      <span key={obj} className="px-1.5 py-0.5 text-[10px] sm:text-[11px] bg-brand-orange/10 dark:bg-brand-orange/20 text-brand-orange rounded-full border border-brand-orange/20 dark:border-brand-orange/30">
                        {obj}
                      </span>
                    ))}
                    {training.objective.length > 2 && <span className="px-1.5 py-0.5 text-[10px] sm:text-[11px] bg-gray-200 dark:bg-gray-700 rounded-full">+ {training.objective.length - 2}</span>}
                  </div>
                </div>
                <button 
                  onClick={() => setSelectedTraining(training)}
                  className="w-full mt-2 bg-brand-orange hover:bg-orange-600 text-white font-semibold py-2 px-3 rounded-lg flex items-center justify-center space-x-1.5 transition-colors text-xs sm:text-sm"
                >
                  <IconEye className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> <span>Detalhes</span>
                </button>
              </div>
            </div>
          ))}
